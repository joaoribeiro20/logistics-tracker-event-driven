...
public class RegisterScanUseCase {
    ...
    public RegisterScanUseCase(TrackingEventRepository trackingEventRepository,
    ...
    public RegisterScanResponse execute(RegisterScanRequest request) {
        ...
        Order order = orderRepository.findById(request.getOrderId())
                .orElseThrow(() -> new IllegalArgumentException("Pedido não encontrado: " + request.getOrderId()));
        ...
        publisher.publishEvent(new ScanRegistradoEvent(
                order.getId(),
                request.getScannedById(),
                request.getStatus(),
                saved.getOccurredAt()
        ));
